...
import { dayMilliseconds, HANGOUT_AVAILABILITY_STAGE, HANGOUT_CONCLUSION_STAGE, HANGOUT_SUGGESTIONS_LIMIT, HANGOUT_SUGGESTIONS_STAGE, HANGOUT_VOTING_STAGE, hourMilliseconds } from '../../src/util/constants';
...
describe('DELETE suggestions/leader', () => {
  it('should reject requests if an authSessionId cookie is not found', async () => {
    const response: SuperTestResponse = await request(app)
      .delete('/api/suggestions/leader')
      .send();
  ...
  it('should reject requests without a suggestion ID, hangoutMember ID, and hangout ID in the URL query string', async () => {
    async function testQueryString(queryString: string): Promise<void> {
      const response: SuperTestResponse = await request(app)
        .delete(`/api/suggestions/leader${queryString}`)
        .set('Cookie', `authSessionId=${generateAuthSessionId()}`)
        .send();
  ...
  it('should reject requests with an invalid suggestion ID', async () => {
    async function testSuggestionId(suggestionId: string): Promise<void> {
      const response: SuperTestResponse = await request(app)
        .delete(`/api/suggestions/leader?suggestionId=${suggestionId}&hangoutMemberId=1&hangoutId=htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013`)
        .set('Cookie', `authSessionId=${generateAuthSessionId()}`)
        .send();
  ...
  it('should reject requests with an invalid hangout member ID', async () => {
    async function testHangoutMemberId(hangoutMemberId: string): Promise<void> {
      const response: SuperTestResponse = await request(app)
        .delete(`/api/suggestions/leader?suggestionId=1&hangoutMemberId=${hangoutMemberId}&hangoutId=htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013`)
        .set('Cookie', `authSessionId=${generateAuthSessionId()}`)
        .send();
  ...
  it('should reject requests with an invalid hangout ID', async () => {
    async function testHangoutId(hangoutId: string): Promise<void> {
      const response: SuperTestResponse = await request(app)
        .delete(`/api/suggestions/leader?suggestionId=1&hangoutMemberId=1&hangoutId=${hangoutId}`)
        .set('Cookie', `authSessionId=${generateAuthSessionId()}`)
        .send();
  ...
  it(`should reject requests if the user's auth session is not found, and remove the authSessionId cookie`, async () => {
    const response: SuperTestResponse = await request(app)
      .delete('/api/suggestions/leader?suggestionId=1&hangoutMemberId=1&hangoutId=htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013')
      .set('Cookie', `authSessionId=dummyAuthSessionIdForTesting1234`)
      .send();
  ...
  it('should reject requests if the user is not the hangout leader', async () => {
    ...
    await dbPool.execute(
      `INSERT INTO suggestions VALUES (${generatePlaceHolders(8)});`,
      [1, 2, 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013', 'Some Title', 'Some suggestion description.', Date.now() + (dayMilliseconds * 4), Date.now() + (dayMilliseconds * 4) + hourMilliseconds, false]
    );
...
describe('GET suggestions', () => {
  ...
  it('should reject requests without a hangout ID and hangout member ID in the URL query string', async () => {
    async function testQueryString(queryString: string): Promise<void> {
      const response: SuperTestResponse = await request(app)
        .get(`/api/suggestions${queryString}`)
        .set('Cookie', `authSessionId=${generateAuthSessionId()}`)
        .send();
  ...
  it('should reject requests with an invalid hangout ID', async () => {
    async function testHangoutId(hangoutId: string): Promise<void> {
      const response: SuperTestResponse = await request(app)
        .get(`/api/suggestions?hangoutId=${hangoutId}&hangoutMemberId=1`)
        .set('Cookie', `authSessionId=${generateAuthSessionId()}`)
        .send();
    ...
    await testHangoutId('');
    await testHangoutId('?');
    await testHangoutId('?hangoutId=htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013');
    await testHangoutId('?hangoutMemberId=1');
  });

  it('should reject requests with an invalid hangout ID', async () => {
    async function testHangoutMemberId(hangoutMemberId: string): Promise<void> {
      const response: SuperTestResponse = await request(app)
        .get(`/api/suggestions?hangoutId=htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013&hangoutMemberId=${hangoutMemberId}`)
        .set('Cookie', `authSessionId=${generateAuthSessionId()}`)
        .send();
  ...
  it(`should reject requests if the user's auth session is not found, and remove the authSessionId cookie`, async () => {
    const response: SuperTestResponse = await request(app)
      .get('/api/suggestions?hangoutId=htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013&hangoutMemberId=1')
      .set('Cookie', `authSessionId=dummyAuthSessionIdForTesting1234`)
      .send();
  ...
  it('should reject requests if no results are found for the hangout ID and hangout member ID provided, purposefully not providing too much details in the response', async () => {
    ...
    expect(response.body.message).toBe(`Hangout not found or you're not a member of it.`);
    expect(response.body.reason).toBe('notHangoutMember');
  });

  it('should accept the request and return the suggestions with the count of likes and votes for each suggestion, as well as the ID of suggestions the user has liked or voted for', async () => {
    await dbPool.execute(
      `INSERT INTO accounts VALUES (${generatePlaceHolders(8)});`,
      [1, 'example@example.com', 'someHashedPassword', 'johnDoe', 'John Doe', Date.now(), true, 0]
    );
    ...
    expect(response.body).toHaveProperty('memberLikes');
    expect(response.body).toHaveProperty('memberVotes');

    expect(Array.isArray(response.body.suggestions)).toBe(true);
    const suggestion = response.body.suggestions[0];

    expect(suggestion).toHaveProperty('suggestion_id');
    expect(Number.isInteger(suggestion.suggestion_id)).toBe(true);

    expect(suggestion).toHaveProperty('hangout_member_id');
    expect(Number.isInteger(suggestion.hangout_member_id)).toBe(true);

    expect(suggestion).toHaveProperty('suggestion_title');
    expect(typeof suggestion.suggestion_title).toBe('string');

    expect(suggestion).toHaveProperty('suggestion_description');
    expect(typeof suggestion.suggestion_description).toBe('string');

    expect(suggestion).toHaveProperty('suggestion_start_timestamp');
    expect(Number.isInteger(suggestion.suggestion_start_timestamp)).toBe(true);

    expect(suggestion).toHaveProperty('suggestion_end_timestamp');
    expect(Number.isInteger(suggestion.suggestion_end_timestamp)).toBe(true);

    expect(suggestion).toHaveProperty('is_edited');
    expect([0, 1]).toContain(suggestion.is_edited);

    expect(suggestion).toHaveProperty('likes_count');
    expect(Number.isInteger(suggestion.likes_count)).toBe(true);

    expect(suggestion).toHaveProperty('votes_count');
    expect(Number.isInteger(suggestion.votes_count)).toBe(true);


    expect(Array.isArray(response.body.memberLikes)).toBe(true);
    expect(Array.isArray(response.body.memberVotes)).toBe(true);

    expect(Number.isInteger(response.body.memberLikes[0])).toBe(true);
    expect(Number.isInteger(response.body.memberVotes[0])).toBe(true);
  });
});

describe('POST suggestions/likes', () => {
  it('should reject requests if an authSessionId cookie is not found', async () => {
    const response: SuperTestResponse = await request(app)
      .post('/api/suggestions/likes')
      .send({});
  ...
  it('should reject requests with missing or incorrect keys', async () => {
    ...
    await testKeys({ suggestionId: 1, hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013' });
    await testKeys({ suggestionId: 1, hangoutMemberId: 1 });
    await testKeys({ suggestionId: 1, hangoutMemberId: 1, hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013', someRandomValue: 23 });
  });
  ...
  it('should reject requests with an invalid hangout ID', async () => {
    ...
    await testHangoutMemberId({ suggestionId: 1, hangoutMemberId: 1, hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR' });
    await testHangoutMemberId({ suggestionId: 1, hangoutMemberId: 1, hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR-1749132719013' });
    await testHangoutMemberId({ suggestionId: 1, hangoutMemberId: 1, hangoutId: '1749132719013_htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR' });
    await testHangoutMemberId({ suggestionId: 1, hangoutMemberId: 1, hangoutId: '1749132719013' });
  });
...
describe('DELETE suggestions/likes', () => {
  it('should reject requests if an authSessionId cookie is not found', async () => {
    const response: SuperTestResponse = await request(app)
      .delete('/api/suggestions/likes')
      .send();
  ...
  it('should reject requests without a suggestion ID, hangout member ID, and hangout ID in the URL query string', async () => {
    async function testQueryString(queryString: string): Promise<void> {
      const response: SuperTestResponse = await request(app)
        .delete(`/api/suggestions/likes${queryString}`)
        .set('Cookie', `authSessionId=${generateAuthSessionId()}`)
        .send();
  ...
  it('should reject requests with an invalid suggestion ID', async () => {
    async function testSuggestionId(suggestionId: string): Promise<void> {
      const response: SuperTestResponse = await request(app)
        .delete(`/api/suggestions/likes?suggestionId=${suggestionId}&hangoutMemberId=1&hangoutId=htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013`)
        .set('Cookie', `authSessionId=${generateAuthSessionId()}`)
        .send();
  ...
  it('should reject requests with an invalid hangout member ID', async () => {
    async function testHangoutMemberId(hangoutMemberId: string): Promise<void> {
      const response: SuperTestResponse = await request(app)
        .delete(`/api/suggestions/likes?suggestionId=1&hangoutMemberId=${hangoutMemberId}&hangoutId=htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013`)
        .set('Cookie', `authSessionId=${generateAuthSessionId()}`)
        .send();
  ...
  it('should reject requests with an invalid hangout ID', async () => {
    async function testHangoutId(hangoutId: string): Promise<void> {
      const response: SuperTestResponse = await request(app)
        .delete(`/api/suggestions/likes?suggestionId=1&hangoutMemberId=1&hangoutId=${hangoutId}`)
        .set('Cookie', `authSessionId=${generateAuthSessionId()}`)
        .send();
  ...
  it(`should reject requests if the user's auth session is not found, and remove the authSessionId cookie`, async () => {
    const response: SuperTestResponse = await request(app)
      .delete('/api/suggestions/likes?suggestionId=1&hangoutMemberId=1&hangoutId=htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013')
      .set('Cookie', `authSessionId=dummyAuthSessionIdForTesting1234`)
      .send();
  ...
  it('should accept the request if the suggestion is not liked without taking further action', async () => {
    await dbPool.execute(
      `INSERT INTO accounts VALUES (${generatePlaceHolders(8)});`,
      [1, 'example@example.com', 'someHashedPassword', 'johnDoe', 'John Doe', Date.now(), true, 0]
    );
  ...
  it('should accept the request, remove the suggestion like, and send a websocket message', async () => {
    await dbPool.execute(
      `INSERT INTO accounts VALUES (${generatePlaceHolders(8)});`,
      [1, 'example@example.com', 'someHashedPassword', 'johnDoe', 'John Doe', Date.now(), true, 0]
    );
