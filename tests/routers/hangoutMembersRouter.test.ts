...
describe('POST hangoutMembers/joinHangout/guest', () => {
  ...
  it('should accept the request, create a guest account, create a hangout member row, create an auth session, add a hangout event, send a websocket message, and return a boolean value on whether an auth session was successfully created', async () => {
    ...
    expect(createdGuestRows).toHaveLength(1);
    expect(createdHangoutMemberRows).toHaveLength(1);
...
describe('DELETE hangoutMembers/kick', () => {
  ...
  it('should accept the request, delete the hangout member, delete any votes or suggestion likes made by them if the hangout is not concluded, delete their account if they are a guest user, add a hangout event, and send a websocket message', async () => {
    ...
    expect(deletedGuestRows).toHaveLength(0);
    expect(deletedHangoutMemberRows).toHaveLength(0);
    expect(deletedSuggestionLikeRows).toHaveLength(0);
...
describe('PATCH hangoutMembers/transferLeadership', () => {
  it('should reject requests if an authSessionId cookie is not found', async () => {
    const response: SuperTestResponse = await request(app)
      .patch('/api/hangoutMembers/transferLeadership')
      .send({});
  ...
  it('should reject requests with missing or incorrect keys', async () => {
    ...
    await testKeys({ hangoutMemberId: 1, newLeaderMemberId: 2 });
    await testKeys({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013', newLeaderMemberId: 2 });
    await testKeys({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013', hangoutMemberId: 1 });
    await testKeys({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013', hangoutMemberId: 1, newLeaderMemberId: 2, someRandomValue: 23 });
  });
  ...
  it('should reject requests with an invalid hangout ID', async () => {
    ...
    await testHangoutId({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR', hangoutMemberId: 1, newLeaderMemberId: 2 });
    await testHangoutId({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR-1749132719013', hangoutMemberId: 1, newLeaderMemberId: 2 });
    await testHangoutId({ hangoutId: '1749132719013_htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR', hangoutMemberId: 1, newLeaderMemberId: 2 });
    await testHangoutId({ hangoutId: '1749132719013', hangoutMemberId: 1, newLeaderMemberId: 2 });
  });
  ...
  it('should reject requests with an invalid hangout member ID', async () => {
    ...
    await testHangoutMemberId({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013', hangoutMemberId: null, newLeaderMemberId: 2 });
    await testHangoutMemberId({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013', hangoutMemberId: 23.5, newLeaderMemberId: 2 });
    await testHangoutMemberId({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013', hangoutMemberId: '23.5', newLeaderMemberId: 2 });
  });

  it('should reject requests with an invalid new leader member ID', async () => {
    async function testNewLeaderHangoutMemberId(requestData: any): Promise<void> {
      const response: SuperTestResponse = await request(app)
        .patch('/api/hangoutMembers/transferLeadership')
        .set('Cookie', `authSessionId=${generateAuthSessionId()}`)
        .send(requestData);
      ...
      expect(response.status).toBe(400);
      expect(response.body).toHaveProperty('message');
      expect(response.body.message).toBe('Invalid new leader hangout member ID.');
    };

    await testNewLeaderHangoutMemberId({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013', hangoutMemberId: 1, newLeaderMemberId: null });
    await testNewLeaderHangoutMemberId({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013', hangoutMemberId: 1, newLeaderMemberId: 23.5 });
    await testNewLeaderHangoutMemberId({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013', hangoutMemberId: 1, newLeaderMemberId: '23.5' });
  });

  it('should reject requests if the user attempts to transfer the leadership to themselves', async () => {
    const response: SuperTestResponse = await request(app)
      .patch('/api/hangoutMembers/transferLeadership')
      .set('Cookie', `authSessionId=${generateAuthSessionId()}`)
      .send({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013', hangoutMemberId: 1, newLeaderMemberId: 1 });

    expect(response.status).toBe(409);
    expect(response.body).toHaveProperty('message');
    expect(response.body.message).toBe(`Can't transfer leadership to yourself.`);
  });
  ...
  it(`should reject requests if the user's auth session is not found, and remove the authSessionId cookie`, async () => {
    const response: SuperTestResponse = await request(app)
      .patch('/api/hangoutMembers/transferLeadership')
      .set('Cookie', `authSessionId=dummyAuthSessionIdForTesting1234`)
      .send({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013', hangoutMemberId: 1, newLeaderMemberId: 2 });
  ...
  it('should reject requests if the new leader is not found in the hangout', async () => {
    await dbPool.execute(
      `INSERT INTO accounts VALUES (${generatePlaceHolders(8)});`,
      [1, 'example@example.com', 'someHashedPassword', 'johnDoe', 'John Doe', Date.now(), true, 0]
    );
    ...
    expect(response.body.message).toBe('Hangout member not found.');
    expect(response.body.reason).toBe('memberNotFound');
  });

  it('should accept the request, transfer the leadership, add a hangout event, and send a websocket message', async () => {
    await dbPool.execute(
      `INSERT INTO accounts VALUES (${generatePlaceHolders(8)}), (${generatePlaceHolders(8)});`,
      [
        1, 'example@example.com', 'someHashedPassword', 'johnDoe', 'John Doe', Date.now(), true, 0,
    ...
    const [updatedRows] = await dbPool.execute<RowDataPacket[]>(
      `SELECT hangout_member_id, is_leader FROM hangout_members WHERE hangout_id = ?;`,
      ['htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013']
    );

    expect(updatedRows[0].hangout_member_id).toBe(1);
    expect(updatedRows[0].is_leader).toBe(0);

    expect(updatedRows[1].hangout_member_id).toBe(2);
    expect(updatedRows[1].is_leader).toBe(1);
...
describe('PATCH hangoutMembers/claimLeadership', () => {
  it('should reject requests if an authSessionId cookie is not found', async () => {
    const response: SuperTestResponse = await request(app)
      .patch('/api/hangoutMembers/claimLeadership')
      .send({});
  ...
  it('should reject requests with missing or incorrect keys', async () => {
    ...
    await testKeys({ hangoutMemberId: 1 });
    await testKeys({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013' });
    await testKeys({ hangoutId: 'htUJOeoHJhuI8O7JA4HZPTBq7e8x7TgR_1749132719013', hangoutMemberId: 1, someRandomValues: 23 });
  });
  ...
  it('should reject requests if the hangout already has a leader, returning their hangout member ID in the response', async () => {
    await dbPool.execute(
      `INSERT INTO accounts VALUES (${generatePlaceHolders(8)}), (${generatePlaceHolders(8)});`,
      [
        1, 'example@example.com', 'someHashedPassword', 'johnDoe', 'John Doe', Date.now(), true, 0,
    ...
    expect(response.body.message).toBe('Hangout already has a leader.');
    expect(response.body.reason).toBe('hangoutHasLeader');

    expect(response.body).toHaveProperty('resData');
    expect(response.body.resData).toHaveProperty('leaderMemberId');
    expect(response.body.resData.leaderMemberId).toBe(2);
  });

  it('should reject requests if the user is already the leader', async () => {
    await dbPool.execute(
      `INSERT INTO accounts VALUES (${generatePlaceHolders(8)}), (${generatePlaceHolders(8)});`,
      [
        1, 'example@example.com', 'someHashedPassword', 'johnDoe', 'John Doe', Date.now(), true, 0,
    ...
    expect(response.body.message).toBe(`You're already the hangout leader.`);
    expect(response.body.reason).toBe('hangoutHasLeader');

    expect(response.body).toHaveProperty('resData');
    expect(response.body.resData).toHaveProperty('leaderMemberId');
    expect(response.body.resData.leaderMemberId).toBe(1);
  });

  it('should accept the request, update the hangout leader, add a hangout event, and send a websocket message', async () => {
    await dbPool.execute(
      `INSERT INTO accounts VALUES (${generatePlaceHolders(8)}), (${generatePlaceHolders(8)});`,
      [
        1, 'example@example.com', 'someHashedPassword', 'johnDoe', 'John Doe', Date.now(), true, 0,
    ...
    expect(updatedRows[0].is_leader).toBe(1);
