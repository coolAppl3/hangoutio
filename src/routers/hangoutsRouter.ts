...
hangoutsRouter.patch('/details/updateTitle', async (req: Request, res: Response) => {
  interface RequestData {
    hangoutId: string,
    hangoutMemberId: number,
    newTitle: string,
  ...
  const expectedKeys: string[] = ['hangoutId', 'hangoutMemberId', 'newTitle'];
  if (undefinedValuesDetected(requestData, expectedKeys)) {
    res.status(400).json({ message: 'Invalid request data.' });
    return;
  };
  ...
  try {
    ...
    const [resultSetHeader] = await dbPool.execute<ResultSetHeader>(
      ...
      WHERE
        hangout_id = ?;`,
      [requestData.newTitle, requestData.hangoutId]
    );
...
hangoutsRouter.get('/details/initial', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [hangoutData] = await dbPool.query<HangoutData>(
      ...
      LIMIT 2;`,
      { hangoutId, hangoutMemberId: requesterHangoutMemberDetails.hangout_member_id, accountId: authSessionDetails.user_type === 'guest' ? 0 : authSessionDetails.user_id }
    );
...
hangoutsRouter.get('/events', async (req: Request, res: Response) => {
  const authSessionId: string | null = getRequestCookie(req, 'authSessionId');
  ...
  try {
    ...
    const [isMemberRows] = await dbPool.execute<ResultSetHeader[]>(
      `SELECT
        1
      FROM
        hangout_members
    ...
    const [hangoutEvents] = await dbPool.execute<RowDataPacket[]>(
      `SELECT
        event_description,
        event_timestamp
      FROM
