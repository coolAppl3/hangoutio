...
exports.availabilitySlotsRouter.post('/', async (req, res) => {
    ...
    const expectedKeys = ['hangoutID', 'slotStartTimestamp', 'slotEndTimestamp'];
    ...
    try {
      ...
      LIMIT ${hangoutValidation_1.hangoutMemberLimit};`, [requestData.hangoutID]);
        ...
        ;
        const memberDetails = hangoutRows.find((member) => member[`${userType}_id`] === userID);
        if (!memberDetails) {
            res.status(401).json({ success: false, message: 'Invalid credentials. Request denied.' });
            return;
        }
      ...
      WHERE
        hangout_member_id = ?
      LIMIT ${availabilitySlotsValidation.availabilitySlotsLimit};`, [memberDetails.hangout_member_id]);
        ...
        VALUES(${(0, generatePlaceHolders_1.generatePlaceHolders)(3)});`, [memberDetails.hangout_member_id, requestData.hangoutID, requestData.slotStartTimestamp, requestData.slotEndTimestamp]);
      ...
      VALUES(${(0, generatePlaceHolders_1.generatePlaceHolders)(3)});`, [memberDetails.hangout_member_id, requestData.slotStartTimestamp, requestData.slotEndTimestamp]);
        await connection.commit();
        res.json({ success: true, resData: { availabilitySlotID: resultSetHeader.insertId } });
    }
    catch (err) {
...
exports.availabilitySlotsRouter.put('/', async (req, res) => {
    ...
    const userID = (0, userUtils_1.getUserID)(authToken);
    const requestData = req.body;
    const expectedKeys = ['hangoutID', 'availabilitySlotID', 'slotStartTimestamp', 'slotEndTimestamp'];
    if ((0, requestValidation_1.undefinedValuesDetected)(requestData, expectedKeys)) {
        res.status(400).json({ success: false, message: 'Invalid request data.' });
        return;
    }
...
exports.availabilitySlotsRouter.delete('/', async (req, res) => {
    ...
    try {
      ...
      LIMIT ${availabilitySlotsValidation.availabilitySlotsLimit};`, [requestData.hangoutMemberID]);
        ...
        ;
        const slotToDelete = availabilitySlotRows.find((slot) => slot.availability_slot_id === requestData.availabilitySlotID);
        if (!slotToDelete) {
            res.status(404).json({ success: false, message: 'Slot not found.' });
            return;
        }
